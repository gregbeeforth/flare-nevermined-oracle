...
describe("createConsumer", () => {
  ...
  it("should parse multiple feed IDs from comma-separated string", () => {
    const ids =
      "0x01464c522f555344000000000000000000,0x014254432f555344000000000000000000"
        .split(",")
        .map((id) => id.trim());
  ...
  it("should use default RPC URL and single feed ID when no args are given", () => {
    const consumer = createConsumer();
    expect(consumer).toBeDefined();
    expect("0x01464c522f555344000000000000000000".split(",")).toHaveLength(1);
  });
});
